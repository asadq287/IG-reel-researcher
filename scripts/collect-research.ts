...
async function collectFromKeyword(searchKeyword: string, target: number): Promise<CollectedReel[]> {
  ...
  const maxScrollAttempts = target * 3;

  while (reels.length < target && scrollAttempts < maxScrollAttempts) {
    ...
    for (const reelId of links) {
      if (seen.has(reelId) || reels.length >= target) continue;
      seen.add(reelId);
      ...
      console.log(`[${reels.length}/${target}] ${reelId}`);
    }
    ...
    console.log(`  scrolling... waiting ${(watch / 1000).toFixed(1)}s`);
